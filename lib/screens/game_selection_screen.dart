...
//
// Экран выбора мини-игры. Открывается когда задание требует мини-игру.
// Показывает три карточки игр, игрок нажимает на любую и переходит в неё.
//
...
// ─── Описание одной игры для карточки ─────────────────────────────────────
class _GameInfo {
  final String id;
  final String title;
...
const _kGames = [
  _GameInfo(
    ...
    description: 'Запомни все предметы. Один исчезнет — успей угадать какой!',
    emoji: '👁️',
  ...
  _GameInfo(
    id: 'scramble',
    title: 'Собери слово',
    description: 'Из перемешанных букв собери правильное слово как можно быстрее!',
    emoji: '🔤',
  ...
  Widget _buildGame(String id) {
    return switch (id) {
      'disappeared' => DisappearedGame(
          items: widget.task.miniGameItems,
          onSuccess: () { Navigator.pop(context); widget.onSuccess(); },
          onFail:    () { Navigator.pop(context); widget.onFail(); },
        ),
        ...
        child: Stack(children: [
          ...
          Padding(
            ...
            child: Column(children: [
                  ...
                  for (int i = 0; i < _kGames.length; i++) ...[
                    ScaleTransition(
                      ...
                      child: FadeTransition(
                        ...
                        child: _GameCard(
                          game: _kGames[i],
                          isSelected: _selectedId == _kGames[i].id,
              ...
              // Подсказка
              Text(
                'Выбери игру — после победы задание будет выполнено',
...
class _GameCard extends StatefulWidget {
  ...
  const _GameCard({required this.game, required this.isSelected, required this.onTap});
...
class _GameCardState extends State<_GameCard> with SingleTickerProviderStateMixin {
  ...
  Color get _difficultyColor => switch (widget.game.difficulty) {
    'Лёгкая'  => kGreen,
  ...
  Widget build(BuildContext context) {
    return ScaleTransition(
      ...
      child: GestureDetector(
        onTapDown: (_) { setState(() => _pressed = true);  _pressCtrl.forward(); },
        onTapUp:   (_) { setState(() => _pressed = false); _pressCtrl.reverse(); widget.onTap(); },
        onTapCancel: () { setState(() => _pressed = false); _pressCtrl.reverse(); },
        child: Container(
          height: 100,
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: [widget.game.colors[0], widget.game.colors[1]],
              begin: Alignment.topLeft, end: Alignment.bottomRight,
            ),
            borderRadius: BorderRadius.circular(22),
            boxShadow: [
              BoxShadow(
                color: widget.game.colors[1].withOpacity(0.35),
                  ...
                  Row(children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: _difficultyColor.withOpacity(0.7), width: 1),
                      ),
